...
void copyVectorRow(vector<vector<short> > &v, int a, int b) { //copy row a to row b
	v[b].clear();
...
void HW_median(ImagePtr I1, int filter, ImagePtr I2)
{
	...
	if (filter % 2 == 0) filter++;                                                               //keep filter odd
	...
	for (int ch = 0; IP_getChannel(I1, ch, in, type); ch++)
	{
		...
		short **buff = new short*[filter];								  //***dynamic array wont let me do short *buff[filter];	
		short **startOfBufferRow = new short*[filter];
		short **startOfKernal = new short*[filter];
		for (int bIndex = 0; bIndex < filter; ++bIndex) {
			buff[bIndex] = paddedImage[bIndex].data();
			startOfBufferRow[bIndex] = buff[bIndex];                      //***Set the reset pointer  
			startOfKernal[bIndex] = buff[bIndex];
		}
		...
		for (int yIncrement = 0; yIncrement < h; ++yIncrement)												// Sliding filter windows vertically direction
		{
			for (int xIncrement = 0; xIncrement < w; ++xIncrement)											// Sliding filter windows horizontally direction
			{
				...
				//get the kernal int the sorted vector to be sorted
				if (xIncrement == 0)                //**                  A1) buffer is filled atm with the correct rows so add everything to the 'sorted' 1D vector and then sort the vector
				{
					for (int buffRow = 0; buffRow < filter; ++buffRow)											// Going through rows of buffer 9shifted the rows at the end)
					{
						for (int buffCol = xIncrement; buffCol < filter + xIncrement; ++buffCol)					// Going through cols of buffer (using the shift from the for loop)
						{
							if (buffCol == (xIncrement + 1)) {                                     //***now i know where the first pixel of kernal will ne located in
								startOfBufferRow[buffRow] = buff[buffRow];
							}
					...
					for (int buffRow = 0; buffRow < filter; ++buffRow)											// Going through rows of buffer 9shifted the rows at the end)
					{
						for (int buffCol = xIncrement; buffCol < filter + xIncrement; ++buffCol)					// Going through cols of buffer (using the shift from the for loop)
						{
							if (buffCol == xIncrement)
								buff[buffRow] = startOfBufferRow[buffRow];                                  //***get back the position of the first pixel of kernal row
			...
			if (yIncrement < h - 1) {
				...
				buff[yIncrement%filter] = paddedImage[filter + yIncrement].data();       //swaping the old row in buffer to the next row in the image
				buff[(yIncrement + 1) % filter] = paddedImage[filter + yIncrement].data();
				buff[(yIncrement + 2) % filter] = paddedImage[filter + yIncrement].data();
			}
		}
